...
struct Score control(int time_limit, int time_bonus) {
    ...
    int crocodile_1, crocodile_2, i_crocodile, reference_crocodile, relative_on_crocodile, on_crocodile = 0;
    
    // Variabili per le granate della rana
    int grenade = 0, terminate_grenade = 0, y_grenade, x_grenade_left, x_grenade_right, flag_coordinates_grenade = 0;

    // Variabili per i proiettili dei coccodrilli
    int bullet = 0, terminate_bullet = 0, y_bullet = maxx, x_bullet = maxx, direction_bullet, i_crocodile_bullet, interval = (rand() % (10 - 5 + 1)) + 5;
    time_t bullet_timer;
    ...
    int den_visited[N_DENS] = {0, 0, 0, 0, 0}, h_den = H_FROG * 2;
    int x_den[N_DENS], x_den_start = 1, in_any_den, count_dens_visited = 0;
    ...
    while (count_lifes > 0 && count_dens_visited != N_DENS) { // Inizia il ciclo della partita, finchè ci sono vite disponibili o non sono state visitate tutte le tane
        ...
        if (new_match) { // Se è un nuovo match
            ...
            // Libera il buffer frog da precedenti coordinate del match precedente
            if (sem_trywait(&sem_full) == 0) {
                temp = buffer[i_read];
                i_read = (i_read + 1) % DIM_BUFFER;
                sem_post(&sem_empty);
            }
            ...
            // Genera direzioni casuali per i coccodrilli e le invia ai threads coccodrillo
            dir_random = rand() % 2;
            for (int i = 0; i < N_CROCODILE; i++) {
                ...
                while (count_generated < 1) {
                    wait_producer(&sem_empty_crocodile);
                    buffer_crocodile[i_write_crocodile] = new;
                    i_write_crocodile = (i_write_crocodile + 1) % DIM_BUFFER_CROCODILE;

                    signal_producer(&sem_full_crocodile);
                    count_generated++;
            ...
            // Se ci sono granate o proiettili in gioco manda un segnale di terminazione dei relativi threads con le coordinate fuori schermo
            if (x_grenade_left > 0 && x_grenade_left < maxx) {
                new.x = maxx;
                new.c = TERMINATE_GRENADE;
                new.dir = direction_bullet;
                while (count_generated < 1) {
                    wait_producer(&sem_empty_grenade);
                    buffer[i_write_grenade] = new;
                    i_write_grenade = (i_write_grenade + 1) % DIM_BUFFER_GRENADE;
                    signal_producer(&sem_full_grenade);
                    count_generated++;
                }
                count_generated = 0;
            }

            if (x_grenade_right > 0 && x_grenade_right < maxx) {
                new.x = -1;
                new.c = TERMINATE_GRENADE;
                new.dir = direction_bullet;
                while (count_generated < 1) {
            ...
            if (x_bullet > 0 && x_bullet < maxx) {
                ...
                while (count_generated < 1) {
                    wait_producer(&sem_empty_bullet);
                    buffer[i_write_bullet] = new;
                    i_write_bullet = (i_write_bullet + 1) % DIM_BUFFER_BULLET;
                    signal_producer(&sem_full_bullet);
                    count_generated++;
                }
                count_generated = 0;
            }
        ...
        // Lettura delle coordinate nel buffer 
        while (count_read < 1) {
            wait_consumer(&sem_full);
            temp = buffer[i_read];
            i_read = (i_read + 1) % DIM_BUFFER;

            signal_consumer(&sem_empty);
            count_read++;
        }
        count_read = 0;
        ...
        if(grenade) { // Se i thread sono stati generati vengono inviate le coordinate x della rana
            for(int i = 0; i < 2; i++) {
                new.x = x_frog;
                new.c = COORDINATE_X_FROG_GRENADE;
                new.dir = i; // Specifica quale dei due thread deve leggere la posizione
                
                while (count_generated < 1) {
                    wait_producer(&sem_empty_grenade);
                    buffer_grenade [i_write_grenade] = new;
                    i_write_grenade = (i_write_grenade + 1) % DIM_BUFFER_GRENADE;
                    signal_producer(&sem_full_grenade);
                    count_generated++;
                }
        ...
        if (in_pavement) { // Se la rana è sul marciapiede
            ...
            if(y_frog == -2) { // Se la rana è alla fine del marciapiede
                ...
                new.c = FROG_RIVER;
                while (count_generated < 1) {
                    wait_producer(&sem_empty_frog);
                    buffer_frog[i_write_frog] = new;   
                    i_write_frog = (i_write_frog + 1) % DIM_BUFFER_FROG;
                    signal_producer(&sem_full_frog);
                    count_generated++;
                }
                count_generated = 0;
        ...
        if (in_river && !in_pavement) { // Se la rana è nel fiume e non sul marciapiede
            ...
            if (on_crocodile ) { // Se la rana è sopra il coccodrillo
                ...
                if ((int)(current_time - bullet_timer) >= interval) { // Se è passato un intervallo di tempo tra i 5 e i 10 secondi
                    ...
                    if ((direction_crocodile[i_crocodile_bullet] == 0 && x_crocodile_end[i_crocodile_bullet] > 0) ||
                        ...
                        if(bullet) { // Se il thread proiettile è stato creato ed è attivo invia le coordinate di partenza
                            new.y = y_frog;
                            ...
                            while (count_generated < 1) {
                                wait_producer(&sem_empty_bullet);
                                buffer_bullet[i_write_bullet] = new;
                                i_write_bullet = (i_write_bullet + 1) % DIM_BUFFER_BULLET;
                                signal_producer(&sem_full_bullet);
                                count_generated++;
                            }
            ...
            // Controlla la collisione tra proiettile e granate, se collidono invia le coordinate di terminazione per entrambi i threads
